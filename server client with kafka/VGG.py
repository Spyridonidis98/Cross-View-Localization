...
def space2channel(x):
    ...
    '''
    B,C,H,W,Z = x.shape
    return x.permute([0, 1, 4, 2, 3]).reshape(B,C*Z,H,W)
...
class ConvBlock(nn.Module):
    def __init__(self, C_in, Cout):
        super(ConvBlock, self).__init__()

        self.layer = nn.Sequential(
            nn.Conv2d(C_in, Cout, kernel_size=3, padding=1, bias=False),
            L2Norm(),#nn.BatchNorm2d(Cout),
...
class Unet(nn.Module):
    def __init__(self, C_in):
        ...
        self.layer = nn.Sequential(
            nn.Conv2d(C_in*8, C_in*4, kernel_size=3, padding=1, bias=False),
            L2Norm(),#nn.BatchNorm2d(Cout),
            nn.ReLU(inplace=False),
            nn.Conv2d(C_in*4, C_in*2, kernel_size=3, padding=1, bias=False),
            L2Norm(),#nn.BatchNorm2d(Cout),
            nn.ReLU(inplace=False)
        )

        self.down0 = ConvBlock(C_in*2, C_in)
        self.down1 = DownBlock(C_in, C_in*2)
        self.down2 = DownBlock(C_in*2, C_in*4)
        self.down3 = DownBlock(C_in*4, C_in*8)

        self.up2 = UpBlock(C_in*8 +  C_in*4, C_in*4)
        self.up1 = UpBlock(C_in*4+C_in*2, C_in*2)
        self.up0 = UpBlock(C_in*2+C_in, C_in)
