...
def grid_sample(image, optical, jac=None):
    ...
    if len(optical.shape) == 4:
        _, H, W, _ = optical.shape #[B, H, W, 2] H=W in satmap

    if len(optical.shape) == 5:
        _, H, W, Z, _ = optical.shape #[B, H, W, 2] H=W in satmap
    ...
    if len(optical.shape) == 5:
        ix = optical[..., 0].view(N, 1, H, W, Z)
        iy = optical[..., 1].view(N, 1, H, W, Z)
    ...
    # test = (iy_nw * IW + ix_nw).long().view(N, 1, H * W).repeat(1, C, 1).cpu().numpy()
    if len(optical.shape) == 4:
    ...
    if len(optical.shape) == 5:
        nw_val = torch.gather(image, 2, (iy_nw * IW + ix_nw).long().view(N, 1, H * W * Z).repeat(1, C, 1)).view(N, C, H, W, Z)
        ne_val = torch.gather(image, 2, (iy_ne * IW + ix_ne).long().view(N, 1, H * W * Z).repeat(1, C, 1)).view(N, C, H, W, Z)
        sw_val = torch.gather(image, 2, (iy_sw * IW + ix_sw).long().view(N, 1, H * W * Z).repeat(1, C, 1)).view(N, C, H, W, Z)
        se_val = torch.gather(image, 2, (iy_se * IW + ix_se).long().view(N, 1, H * W * Z).repeat(1, C, 1)).view(N, C, H, W, Z)
